	...
	for i, p := range op.decoded.Patches {
		...
		if err := patcher.ApplyPatch(p); err != nil {
			...
			if errors.As(err, &notFoundError) {
				...
				if notFoundError.Path.ObjectID != nil {
					op.err.AdditionalInfo = &pb.Error_AdditionalInfo{
						ObjectId: uint64(*notFoundError.Path.ObjectID),
					}
				}
			} else if errors.As(err, &outOfStockError) {
				op.err = &pb.Error{
					Code:    pb.ErrorCodes_OUT_OF_STOCK,
					Message: outOfStockError.Error(),
					AdditionalInfo: &pb.Error_AdditionalInfo{
						ObjectId: outOfStockError.ListingID,
					},
				}
			} else {
...
func (op *SubscriptionRequestOp) process(r *Relay) {
	...
	for _, filter := range op.im.Filters {
		...
		case pb.ObjectType_OBJECT_TYPE_ACCOUNT:
			where = ` (p.objectType='Accounts')`
		case pb.ObjectType_OBJECT_TYPE_INVENTORY:
			where = ` (p.objectType='Inventory')`
		case pb.ObjectType_OBJECT_TYPE_LISTING:
			where = ` (p.objectType='Listings')`
		case pb.ObjectType_OBJECT_TYPE_TAG:
			where = ` (p.objectType='Tags')`
		case pb.ObjectType_OBJECT_TYPE_ORDER:
			where = ` (p.objectType='Orders')`
...
func (op *KeyCardEnrolledInternalOp) process(r *Relay) {
	...
	if !isNewShop {
		...
		if op.keyCardIsGuest {
			op.done <- errors.New("guest keycards are not allowed for new shops")
	...
	var value any
	var fields []any
	var patchOp patch.OpString
	_, has := r.shopIDsToShopState.MustGet(shopID).data.Accounts.Get(op.userWallet.Address[:])
	// if this an existing account, we just append the keycard to the list
	if has {
		value = op.keyCardPublicKey
		fields = []any{"KeyCards"}
		patchOp = patch.AppendOp
	} else {
		// if this is a new account, we create a new account and add the keycard to the list
