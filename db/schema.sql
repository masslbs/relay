...
create type objectTypeEnum as enum ('Manifest', 'Listings', 'Tags',  'Orders', 'Inventory', 'Accounts');
...
create table patches (
    ...
    op text NOT NULL CHECK (op IN ('add', 'append', 'replace', 'remove', 'increment', 'decrement')),
...
alter table patches add constraint patchesObjectId check (
    (objectType in ('Listings', 'Orders', 'Inventory') and octet_length(objectId) = 8) or
    (objectType = 'Manifest' and objectId is null) or
    (objectType = 'Accounts' and objectId is null) or
    (objectType = 'Tags' and objectId is null)
);
alter table patches add constraint patchesAccountAddr check (
    (objectType = 'Accounts' and octet_length(accountAddr) = 20) or
    (objectType != 'Accounts' and accountAddr is null)
);
alter table patches add constraint patchesTagName check (
    (objectType = 'Tags' and tagName is not null and length(tagName) > 0) or
    (objectType != 'Tags' and tagName is null)
);
...
CREATE TABLE payments (
    ...
    orderExpiresAt   TIMESTAMP, -- set once the order is entering locked payment state
    ...
    -- set if for e.g. a user cancels it, the timer expired or a variation was removed
