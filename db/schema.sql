...
alter table patchSets add constraint patchSetsRootHash check (octet_length(rootHash) = 32);
...
create index patchesOnAccountId on patches(accountId) where accountId is not null;
create index patchesOnTagName on patches(tagName) where tagName is not null;
